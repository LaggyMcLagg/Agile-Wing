<div class="container mt-5">
            ...
            <div class="row mb-3">
                <div class="col-md-6">
                    ...
                    <select class="form-select @error('courseClass_id') is-invalid @enderror" id="courseClass_id" name="courseClass_id" >
                        ...
                        <option value="{{ $courseClass->id }}"@if($courseClass->id == $hourBlockCourseClass->course_class_id) selected @endif>{{ $courseClass->id }}</option>
                        @endforeach
                    </select>
 ...
 @error('hourEnd') is-invalid @enderror" value="{{ $courseClass->hour_beginning }}">
                @error('hourEnd')
                <span class="invalid-feedback" role="alert">
                    <strong>{{ $message }}</strong>
                </span>
            ...
            <div class="form-group">
                ...
                <input step="1" type="time" id="hourEnd" courseID="hourEnd" autocomplete="hourEnd" class="form-control
 @error('hourEnd') is-invalid @enderror" value="{{ $courseClass->hour_end }}">
                @error('hourEnd')
                <span class="invalid-feedback" role="alert">
                    <strong>{{ $message }}</strong>
                </span>
